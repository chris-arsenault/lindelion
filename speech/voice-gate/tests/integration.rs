...
fn passes_general_battery() {
    // Latency is scoped out (ADR-0018); the gate reports zero latency and is validated above.
    let mut effect = VoiceGate::new();
...
fn inference_allocation_is_bounded() {
    // ADR-0018: inference may allocate a bounded amount inline. The block spans several VAD chunks
