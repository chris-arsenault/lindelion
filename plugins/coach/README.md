...
It targets **Windows only** — Windows VST3 build + a **Vizia** editor on the shared `lindelion-ui`
stack — per [ADR-0023](../../docs/adr/0023-new-vsts-windows-only.md), and reuses Cenedril's M0/M1
Windows platform foundation (the Windows VST3 build path and the Windows `IPlugView`→`HWND`
Vizia/baseview editor attach). It runs in the Galad host and Windows DAWs.

It is **not yet a workspace member** — a reserved home with intent only, so the build and `make ci`
are untouched. When created, the cross-platform delivery-analysis DSP and Vizia view logic are
tested in `make ci`; only the `IPlugView`→`HWND` baseview attach and the Windows bundle are
Windows-gated.
