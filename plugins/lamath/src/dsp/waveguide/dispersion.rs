...
mod tests {
    ...
    #[test]
    fn tube_and_zero_dispersion_bypass() {
        // The Tube never disperses, and an explicit zero stiffness bypasses.
        ...
        let harmonic_string = dispersion_profile(
            48_000.0,
            WaveguideParams {
        ...
        assert!(!harmonic_string.is_enabled());
        assert_eq!(harmonic_string.delay_compensation_samples, 0.0);
    }

    #[test]
    fn default_string_disperses() {
        // M11 P4: the default String now has a (light) stiffness, so its dispersion
        // cascade is engaged and compensates the loop length to stay in tune.
        let default_profile = dispersion_profile(48_000.0, WaveguideParams::default());
        assert!(default_profile.is_enabled());
        assert!(default_profile.delay_compensation_samples > 0.0);
    }
    ...
    fn string_profile_maps_stiffness_to_negative_allpass_sections() {
        ...
        assert!(stiff.coefficient < soft.coefficient);
