...
use crate::{
    DSP_FALLBACK_SAMPLE_RATE, LOOP_FILTER_Q, LOOP_GAIN, PICKUP_POSITION, STRIKE_POSITION,
    model::String1dParams,
};
...
pub(super) fn waveguide_geometry(strike_position: f32, pickup_position: f32) -> WaveguideGeometry {
    let pickup_position = PICKUP_POSITION.clamp(pickup_position);
    WaveguideGeometry {
        pickup_position,
...
fn loop_filter_coefficients(
    ...
    let q = LOOP_FILTER_Q.q_for_resonance(loop_filter_resonance);
