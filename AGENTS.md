...
| `host/` | Galad — standalone Windows realtime VST3 host application (workspace member `galad`, target-gated, excluded from `make ci`): native WASAPI audio I/O, host-side VST3 protocol, a persistent-instance-pool chain with a lock-free control→audio hand-off, opaque-state session persistence, hosted `IPlugView` editor windows, in-process plugin validation, and a **Vizia (winit) UI**. Neutral core tested on Linux; Windows build cross-compiled via cargo-xwin. See [host/README.md](host/README.md) and [ADR-0022](docs/adr/0022-windows-vst3-host.md), [ADR-0024](docs/adr/0024-galad-ui-vizia.md), [ADR-0025](docs/adr/0025-galad-chain-edit-state-pool.md), [ADR-0026](docs/adr/0026-galad-in-process-plugin-containment.md). |
