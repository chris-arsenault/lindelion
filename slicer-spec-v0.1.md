# Melodic Sample Slicer — v0.1 Design Spec

**Working title:** TBD (see §17). Candidates: *Tessera*, *Disjecta*, *Augur*, *Cleave*, *Schism*, *Marrow*, *Stutter*.
**Repo:** same Cargo workspace as the resonator synth (Ahara plugin suite).
...
- **Workspace integration.** Shares utility crates with the resonator synth (plugin shell, sample library, DSP utils, UI widgets).
...
- Source samples ingested into the shared `ahara-sample-library` (SQLite-indexed, blake3-hashed, drag-drop persisting to library). Slicing one sample doesn't lock the sample to this patch; the same source can be re-used in another patch with different markers.
...
| **Pitch-Stability**      | Highly tonal monophonic material                  | Detects pitch contour via pYIN; segments where stable pitch periods break.             |
...
1. **Pitch period analysis** (offline, at slice creation): run pYIN on the slice to detect pitch contour and mark pitch epochs (period boundaries).
...
For each slice, the plugin runs pitch detection (pYIN) on the slice's content and displays:
...
This plugin lives in the same Cargo workspace as the resonator synth. Crate inventory:

```
ahara-plugins/
├── Cargo.toml                    # Workspace root
├── crates/
│   ├── ahara-plugin-shell/       # VST3 + baseview + Vizia integration
│   ├── ahara-sample-library/     # SQLite-indexed sample library
│   ├── ahara-dsp-utils/          # Biquads, SVF, saturation, smoothing
│   ├── ahara-ui/                 # Common Vizia widgets
│   ├── ahara-onset-detect/       # NEW: SuperFlux, ComplexFlux, spectral sparsity, pitch-stability
│   └── ahara-psola/              # NEW: PSOLA pitch shift + pYIN pitch detection
├── plugins/
│   ├── resonator-synth/          # The first plugin
│   └── slicer/                   # This plugin
```

**Shared crates this plugin pulls in:**

- `ahara-plugin-shell` — fully reused for VST3 integration, parameter system, MIDI normalization, state I/O.
- `ahara-sample-library` — fully reused; samples ingested by either plugin appear in the other's library browser.
- `ahara-dsp-utils` — biquads (filter), smoothing (parameter smoothing), envelope helpers.
- `ahara-ui` — knob, slider, waveform view (extended in this plugin to support markers), sample browser.

**New crates introduced:**

- `ahara-onset-detect` — `OnsetDetector` trait with implementations: `SuperFlux`, `ComplexFlux`, `SpectralSparsity`, `PitchStability`, `EnergyFlux`, `ManualGrid`. Each is independently usable. Reusable later for the resonator synth's v2 audio-expression-analysis pipeline.
- `ahara-psola` — PSOLA pitch shift + pYIN pitch detection. Reusable for any future plugin needing formant-preserving pitch manipulation of monophonic content.
...
- Patches stored as TOML in `Patches/Slicer/` (separate subdirectory from resonator patches).
...
- Cached analysis (pYIN epoch positions per slice) **not** stored in patch — recomputed on patch load, since it's deterministic from the source sample + markers and recomputation is cheap.
- DAW state save/restore via `IComponent::getState` / `setState` — same path as resonator synth (postcard binary serialization of active patch).

---

## 14. Technology Stack

Same stack as the resonator synth (see resonator spec §12), with additions:

| Layer            | Choice                                       | Notes                                                              |
| ---------------- | -------------------------------------------- | ------------------------------------------------------------------ |
| Plugin shell     | `ahara-plugin-shell` (shared)                | VST3 + baseview + Vizia integration.                               |
| UI               | `vizia` direct (shared `ahara-ui`)           | Waveform view extended for marker handles.                         |
| Format           | VST3 only                                    | Same constraints as resonator.                                     |
| Onset detection  | `ahara-onset-detect` (this plugin introduces) | All algorithms pure-Rust.                                          |
| Pitch shift      | `ahara-psola` (this plugin introduces)       | Hand-rolled PSOLA + pYIN.                                          |
| FFT              | `realfft`                                    | Used by spectral detection methods + pYIN.                         |
| Sample library   | `ahara-sample-library` (shared)              | Same DB, same on-disk layout.                                      |
...
Slice detection and pYIN run offline (at sample load or marker edit), not in the audio thread. Detection on a 30-second sample completes well under a second on M-series silicon for any of the algorithms.
...
1. **Name.** Working title. Candidates: *Tessera* (mosaic tile, fits the chopping/recombination metaphor), *Disjecta* (scattered fragments, Latin), *Augur* (interpretation of signs, fits worldbuilding aesthetic), *Cleave* (cut + cling double meaning), *Schism*, *Marrow*, *Stutter*. Aesthetic-pair with the resonator synth name should be considered (e.g., *Sympathy* + *Disjecta* both Latin-rooted single-word; *Hollow* + *Marrow* both anatomical).

2. **Bank expansion in v1?** Spec'd as 16 slices for v1, bank expansion as v2. If you'd rather have 4 banks of 16 (64 slices) on day one, the marker storage and UI just needs to grow accordingly — not a deep architectural change. Worth a check before locking.

3. **Slice audition on click in waveform view.** Should audition trigger only when no MIDI note is currently held (to avoid muddying the mix), or always play?

4. **End-offset vs region-based slice boundaries.** Spec uses start-marker-only with per-slice `end_offset_ms`. Alternative: every slice has explicit start AND end markers, allowing slices to overlap or leave gaps. Start-only is simpler and matches your stated preference, but worth confirming once you've thought about whether you'd want non-contiguous slice playback.

5. **Choke group beyond per-pad mono.** Drum machines often allow cross-pad choke (open hi-hat chokes closed hi-hat). Useful for wind material? E.g., two articulations of the same note in a choke group so only one plays at once.

---

## 18. Build Order

Each step's done-criteria is concrete and observable. No time estimates.

### Step 1 — Workspace setup

- Convert the resonator synth's repo into a Cargo workspace.
- Extract `ahara-plugin-shell`, `ahara-sample-library`, `ahara-dsp-utils`, `ahara-ui` as separate crates.
- Resonator synth still builds and runs against the extracted crates.
- New empty crates for `ahara-onset-detect` and `ahara-psola`.
- New empty `plugins/slicer/` crate that produces a `.vst3` bundle and loads in Ableton.

**Done:** both plugins build from the same workspace; Ableton sees the slicer as a separate plugin (no slicing yet).

### Step 2 — Sample loading + waveform view

- Drag-and-drop a sample onto the slicer.
- Sample loads into the shared library.
- Waveform view in the UI renders the loaded sample.

**Done:** drop a clarinet recording, see it as a waveform in the plugin.

### Step 3 — Energy/transient detection + manual markers

- Energy-based detector implemented in `ahara-onset-detect` (the simplest algorithm to validate the pipeline).
...
**Done:** load a sample, detect 16 markers, manually edit a few, see the slice list update.
...
**Done:** load a sample, see markers, trigger pads via a MIDI keyboard, hear slices play back at original pitch.
...
**Done:** chop a sample, set ADSR per slice, hear envelope shaping; gated mode releases on note-off.
...
**Done:** switch detection algorithms on a wind sample; see SuperFlux find onsets that energy-based missed.

### Step 7 — PSOLA pitch shift + pYIN

- `ahara-psola` crate: pYIN pitch detection (offline, cached per slice) + PSOLA real-time synthesis.
...
**Done:** select a clarinet slice, shift +50 cents, hear pitch change without timbral chipmunk effect.

### Step 8 — Tuner + scale snap

- pYIN-based fundamental detection per slice, displayed as note + cents from A=440.
...
**Done:** load a slightly-out-of-tune clarinet recording, tune all slices to A=440, then snap all to A-major; verify the slices play as a tuned scale.
...
**Done:** select a slice, switch to chromatic mode, play a chord on a MIDI keyboard, hear it played in the slice's timbre at three different pitches simultaneously.

### Step 10 — Patch save/load, library integration polish

- Patches saved as TOML, samples referenced by hash with path fallback (same as resonator).
- Library browser drawer shows samples ingested across both plugins.

**Done:** save a patch, close DAW, reopen project, patch reloads with all markers and slice params intact. Verify sample-not-found behavior (move sample on disk, reload, see missing-sample warning).
...
**Done:** plugin passes validator; sustains 16-voice polyphony with headroom on M-series silicon.
...
- **pYIN:** probabilistic variant of the YIN pitch detection algorithm; robust monophonic fundamental estimator.
