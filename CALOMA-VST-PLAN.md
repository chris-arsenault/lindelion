...
in [ADR-0016](docs/adr/0016-caloma-speech-vst-packaging.md); it refines
...
| D3 | ConvolutionReverb | **RESOLVED: dropped** from the product (ADR-0016) |
...
- **NN effects:** DFN3 denoiser + Silero voice-gate run their own inline inference (ADR-0014); they
...
### M0 — crate scaffold + the patch model  [depends on: ADR-0016 ✓]
- ADR-0016 is written + indexed (decision resolved: Calóma; ConvolutionReverb dropped). No gate
  here — proceed.
...
- **Resolved:** D1 name = **Calóma** (`plugins/caloma`); D3 = **ConvolutionReverb dropped**;
  packaging shape recorded in [ADR-0016](docs/adr/0016-caloma-speech-vst-packaging.md).
...
- **Durable docs created:** ADR-0016 (+ index), `plugins/caloma/README.md` (reserved, not yet a
