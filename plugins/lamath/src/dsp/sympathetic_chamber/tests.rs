...
fn chamber_send_scales_super_linearly_with_energy() {
    ...
    // `SYMPATHETIC_SEND_ENERGY_REF` (0.2, the post-P9-makeup output-mix level): soft
    // sits partway up the curve, loud near full send.
    let mut soft = SympatheticChamber::new(SR);
    soft.set_depth(1.0);
    soft.note_on(60);
    let soft_out = render_ring(&mut soft, midi_note_to_hz(60.0), 0.05, 24_000, 8_000);

    let mut loud = SympatheticChamber::new(SR);
    loud.set_depth(1.0);
    loud.note_on(60);
    let loud_out = render_ring(&mut loud, midi_note_to_hz(60.0), 0.25, 24_000, 8_000);
