...
2. **DFN3 Speech Denoiser (inline, native ONNX Runtime).**  [depends on #1] — **DONE**
   ...
   - Implementation: `impl Effect`; `Session` built once at `prepare`; one run per 480-sample hop
     ...
     48 kHz auto-bypass; latency reported as 1920 samples (four hops); inline per ADR-0018.
...
4. **Wire one speech effect to the new fixture (incremental).**  [depends on #2]  **`[DECISION]`** (which effect)
   ...
   - Reference: the effect's existing fixture-test pattern (decode → resample to 48 kHz → process).
     ...
     `#[ignore]`d integration suite (`make test-models`), not `make ci` (ADR-0018 convention).
