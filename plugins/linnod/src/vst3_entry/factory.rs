...
use super::{LinnodVst3Controller, LinnodVst3Processor, SUBCATEGORY};

const CONTROLLER_NAME: &str = VST3_BUNDLE_METADATA.controller_name;

const LINNOD_VST3_CLASSES: &[Vst3ClassRegistration] = &[
    Vst3ClassRegistration::audio_processor(
        LinnodVst3Processor::CID,
        DESCRIPTOR.name,
        SUBCATEGORY,
        create_processor,
    ),
    Vst3ClassRegistration::edit_controller(
        LinnodVst3Controller::CID,
        CONTROLLER_NAME,
        create_controller,
    ),
];

fn linnod_vst3_factory() -> Vst3PluginFactory {
    Vst3PluginFactory::new(&DESCRIPTOR, LINNOD_VST3_CLASSES)
}

fn create_processor() -> ComPtr<FUnknown> {
    ComWrapper::new(LinnodVst3Processor::new())
        .to_com_ptr::<FUnknown>()
        .expect("processor must expose FUnknown")
}

fn create_controller() -> ComPtr<FUnknown> {
    ComWrapper::new(LinnodVst3Controller::new())
        .to_com_ptr::<FUnknown>()
        .expect("controller must expose FUnknown")
}

lindelion_plugin_shell::export_vst3_entrypoints!(linnod_vst3_factory());
...
mod tests {
    ...
    #[test]
    fn linnod_registers_processor_and_controller_with_shared_factory() {
        let factory = linnod_vst3_factory();
        ...
        assert_eq!(processor.cid, LinnodVst3Processor::CID);
        ...
        assert_eq!(controller.cid, LinnodVst3Controller::CID);
