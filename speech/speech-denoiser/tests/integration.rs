...
//!
//! The denoiser runs inference inline (ADR-0018), so it is held to the bounded-allocation bar,
...
fn improves_snr_on_noisy_speech() {
    ...
    // general battery cannot, see ADR-0018).
    assert!(
...
fn passes_general_battery() {
    // Latency is scoped out (ADR-0018): the model's warm-up transient produces output before its
...
fn inference_allocation_is_bounded() {
    // ADR-0018: NN inference may allocate a bounded amount inline. The block spans two hops so an
