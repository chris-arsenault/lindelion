...
| Cenedril | Windows-only passthrough Visualizer VST3 — magnitude + reassigned spectrogram, level/LUFS meters, analysis-signal panel, view/scale/colormap/range controls with persisted settings | [Spec](plugins/cenedril.md), [backlog](plugins/cenedril-backlog.md), [README](../plugins/cenedril/README.md) |
| Lúmedir | Windows-only passthrough Speech-Coach VST3 — bit-exact 0-latency passthrough + off-thread delivery analysis (rate/WPM, pitch dynamism, pauses, clarity) scored against configurable target bands; Vizia live readout + session summary | [Spec](plugins/lumedir.md), [DSP](dsp/delivery-metrics.md), [README](../plugins/lumedir/README.md) |
