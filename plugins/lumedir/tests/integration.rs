...
//!
//! This is gated behind `test-sync-analysis` (→ `lindelion-speech-signals/sync-analysis`), which
//! runs the analyzer **inline on `push`** so `latest_snapshot()` deterministically reflects the fed
