...
| Lúmedir | Quenya `lúme` "time/rhythm" + `-dir` "keeper" (working name) | Windows-only passthrough Speech-Coach VST3 (built M0–M3): bit-exact 0-latency passthrough feeding an off-thread `DeliveryWorker` that computes speaking rate/cadence, pitch dynamism, pause structure, and clarity (crate `plugins/lumedir`). The Vizia live-readout view (M4+) is a later milestone. Windows-only per [ADR-0023](docs/adr/0023-new-vsts-windows-only.md), plan at `LUMEDIR-VST-PLAN.md` |
...
| `plugins/lumedir` | Lúmedir — Windows-only passthrough Speech-Coach VST3, two-class VST3 (`vst3_entry/`), placeholder Vizia editor. Bit-exact 0-latency passthrough that feeds a mono mix to an off-thread delivery worker (`delivery_worker.rs`) over `lindelion-speech-signals`; the worker runs the M1–M3 delivery-metric estimators — speaking rate/cadence (`speaking_rate.rs`), pitch dynamism (`pitch_dynamism.rs`), pause structure (`pause_structure.rs`), clarity (`clarity.rs`) — aggregated into a `Delivery` snapshot (`delivery.rs`). Readout/summary editor views are later milestones. See [ADR-0023](docs/adr/0023-new-vsts-windows-only.md). |
