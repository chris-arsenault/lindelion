...
- **Doc surface.** Each phase's lasting architectural decision (bow driver, dual-polarization, mesh
  ...
  **ADR-0029+**, via repo-docs), with the calibrated targets recorded; one `CHANGELOG.md` line per
...
sync); write the phase ADRs (reserve ADR-0029+) and `CHANGELOG.md`; delegate doc conventions to repo-docs.
...
- **ADR-0029** (M11 gain staging: per-family makeup, per-driver trim, master soft-clip) and **ADR-0030**
