# Glirdir — Sing-to-MIDI Scratchpad — v0.1 Design Spec

**Name etymology:** Sindarin, *singer* / *song-bearer*. From *glir-* (to sing) + *-dir* (agentive masculine).
...
- **Save to Library** button — separately ingests the current scratchpad audio into the shared `lindelion-sample-library` as a regular sample (useful when a take is worth keeping as source material for the slicer or resonator synth).
...
| Onset detect     | `lindelion-onset-detect` (shared from slicer)         | SuperFlux + pitch-stability used here.                         |
...
└── plugins/
    ├── lome/                               # Resonator synth (placeholder name)
    ├── tessera/                            # Slicer (placeholder name)
    └── glirdir/                            # This plugin
```

The slicer spec's PSOLA implementation needs a small adjustment: since SwiftF0 doesn't output per-period epochs (it's frame-based, not period-based), PSOLA's epoch detection becomes self-contained within `lindelion-psola` — peak-picking on per-period autocorrelation windows informed by SwiftF0's pitch contour. Spec update for the slicer deferred until that plugin's implementation pass.
...
Concrete observable definitions of done per step.
...
**Done:** drag works reliably in Ableton on Mac. If broken, evaluate fallback options before proceeding (see §7.3).
...
**Done:** arm the plugin, hit play in Ableton, sing into the input, see the captured waveform appear after N bars.
...
**Done:** capture audio, see a smooth pitch line drawn over the waveform with low-confidence regions visually distinguished. Confirm no denormal/NaN artifacts in pathological inputs (silence, clipped audio, pure noise).
...
**Done:** sing a melody, see piano-roll notes appear matching what was sung (uncleanly, but visibly).
...
**Done:** sing a sloppy melody in C, set key=C scale=Major snap=Hard, see notes snap to scale; toggle snap modes and watch the piano-roll change.
...
**Done:** sing dynamically, set velocity amount = 1, see varied note heights; set amount = 0, see uniform notes.
...
**Done:** hit play on audition, hear the captured/quantized melody back as sine tones; adjust grid strength while playing, hear timing change.
...
**Done:** capture, audition, adjust quantization, drag the MIDI preview onto an Ableton track, see a clean MIDI clip appear.
...
**Done:** capture, save Ableton project, close, reopen → plugin restores with audio and derived MIDI ready to re-derive/drag.
...
**Done:** plugin passes validator; 16-bar capture re-derives MIDI in under 100ms on M-series silicon; UI is responsive during re-derivation.
