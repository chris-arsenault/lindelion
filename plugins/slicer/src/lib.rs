use ahara_onset_detect::{AlgorithmParams, DetectionAlgorithm, DetectionConfig, SliceMarker};
use ahara_plugin_shell::{
    AudioPlugin, ParameterInfo, ParameterRange, PluginDescriptor, PluginState, ProcessContext,
    ProcessSetup,
};
use ahara_psola::{PitchAnalysis, PitchShift};
use ahara_sample_library::SampleReference;
use ahara_ui::{PadId, TriggerMode};

pub const DESCRIPTOR: PluginDescriptor =
    PluginDescriptor::instrument("Ahara Slicer", *b"ahara_slicer_v01");
...
impl Slicer {
    pub fn patch(&self) -> &SlicerPatch {
        &self.patch
    }
}

impl AudioPlugin for Slicer {
