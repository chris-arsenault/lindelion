...
// Driver / contact / surrounding parameter-range tests live in a separate file to keep
// this one under the repository file-size limit; `include!` keeps them in this test
// module verbatim (sharing its helpers and imports).
include!("tests/driver_contact_ranges.rs");
...
fn prepare_patch_for_binding(patch: &mut ResonatorSynthPatch, binding: ParameterBinding) {
    ...
    if let ParameterPath::Driver(parameter) = binding.path() {
        match parameter {
            ...
            DriverParameter::Bow(_) => patch.driver = DriverConfig::Bow(BowConfig::default()),
