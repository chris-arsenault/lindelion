...
- **File(s):** verification only, plus the doc surface: `CHANGELOG.md`, and a new ADR (next free number is
  **ADR-0025**) if step 1's choices are lasting (sympathetic routing especially) — delegate to the
  **repo-docs** skill for the ADR + index row, as M9 did with ADR-0024. Update the Lamath backlog line if
...
- **Change:** one curated `CHANGELOG.md` line (the waveguide/instrument gains effort/energy-scaled
  ...
  defeatable and off by default); write ADR-0025 if warranted; reconcile the backlog item.
