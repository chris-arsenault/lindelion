...
| `host/` | Galad — standalone Windows realtime VST3 host application (workspace member `galad`, target-gated, excluded from `make ci`): WASAPI audio I/O, host-side VST3 protocol, device management, **Vizia (winit) UI** (ADR-0024). M0–M5 done (session model, VST3 host spike, WASAPI engine, chain w/ lock-free hand-off, state/session persistence, IPlugView editor hosting); M6 is the Vizia host UI. See [ADR-0022](docs/adr/0022-windows-vst3-host.md). |
