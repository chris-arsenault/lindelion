//! Limiter against the shared general-signal fidelity battery.
...
fn passes_general_battery() {
    let mut effect = Limiter::new();
    run_general_battery(&mut effect, 48_000.0).expect("limiter passes the general battery");
}
