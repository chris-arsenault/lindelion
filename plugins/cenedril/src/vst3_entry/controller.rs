...
/// Minimal edit controller: Cenedril exposes no parameters in M0. `createView` returns the
/// `FixedSizePlugView`; the actual Vizia editor attaches to the host child window on Windows
/// (M1, ADR-0023) and is a no-op on other platforms.
pub(super) struct CenedrilVst3Controller;

impl Class for CenedrilVst3Controller {
    type Interfaces = (IEditController,);
}

impl CenedrilVst3Controller {
...
impl IPluginBaseTrait for CenedrilVst3Controller {
...
impl IEditControllerTrait for CenedrilVst3Controller {
